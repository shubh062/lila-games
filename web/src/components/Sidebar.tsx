...
import { Map, Calendar, Users, Cpu, Activity, Flame, Eye, EyeOff, Package, Skull, Crosshair, Zap, User, Search } from "lucide-react";
...
export default function Sidebar({
  ...
  }, [index, selectedMap, selectedDate, searchQuery]);
  ...
  return (
    <aside className="w-80 bg-zinc-950 flex flex-col shrink-0 text-sm overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="p-2 border-b border-zinc-800/50 bg-zinc-950/80 backdrop-blur-sm z-10">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-zinc-900 border border-zinc-800 rounded-md py-1.5 pl-8 pr-3 text-xs text-zinc-300 placeholder:text-zinc-600 outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 transition-all"
            />
          </div>
          <div className="flex items-center justify-between text-xs text-zinc-500 font-medium mt-2 px-1">
        ...
        {filteredMatches.length === 0 && (
          <div className="text-center p-8 text-zinc-500 text-sm">
            {searchQuery ? "No matches found for this ID." : "No matches found for these filters."}
          </div>
        )}
        </div>
      </div>
